    ...
    where TCommand : IRequest<object?>
{
    ...
    private void CreateEndpoint(IEndpointRouteBuilder app, string route, string parameterName)
    {
        ...
        {
            case "id":
                app.MapDelete(route, async ([FromRoute(Name = "id")] TKey key, [FromServices] IRequestSender sender, CancellationToken cancellationToken) 
                    => await HandleAsync(key, sender, cancellationToken))
                    .DisableAntiforgery();
                break;
            case "code":
                app.MapDelete(route, async ([FromRoute(Name = "code")] TKey key, [FromServices] IRequestSender sender, CancellationToken cancellationToken) 
                    => await HandleAsync(key, sender, cancellationToken))
                    .DisableAntiforgery();
                break;
            default:
